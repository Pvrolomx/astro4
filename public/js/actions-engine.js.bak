...
const warnings = {
  ...
  en: [
    "Impulsive money decisions",
    "Unnecessary arguments",
    "Commitments you can't keep",
    "Excess caffeine or stimulants",
    "Comparing yourself to others on social media",
    "Postponing important things for urgent ones",
...
function generateActions(western, chinese, numerology, birthDate, lang) {
  ...
  const warningIdx = Math.floor(seededRandom(uniqueSeed, today.getDate() * 50) * warnings[lang].length);
  const colorIdx = Math.floor(seededRandom(uniqueSeed, 200 + today.getDate()) * colors[lang].length);
  
  return {
    actions: actions,
    warning: warnings[lang][warningIdx],
